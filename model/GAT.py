...
class GATEncoder(torch.nn.Module):
    """GCN组成的编码器"""
    def __init__(self, in_channels, hidden, out_channels, heads):
        super().__init__()
        self.conv1 = GATConv(in_channels=in_channels, out_channels=hidden, heads=heads)
        self.conv2 = GATConv(in_channels=hidden * heads, out_channels=out_channels, heads=1)
...
def get_embedding(x, edge_index, dim=16, hidden=8, heads=4, lr=0.01, epochs=1000, verbose=1, test_edges=None, test_edges_false=None):
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    x = x.to(device)
    edge_index = edge_index.to(device)
    input = x.shape[1]
    model = GAE(GATEncoder(input, hidden, dim, heads))
